...
packethandlers.HandleIncoming0x28 = function(e, messages)
    ...
    -- Keep the current six-bit ToAU action-header parser, while passing the
    -- original five-slot warning queue into SimpleLog's reference alert path.
    local parsed = gActionHandlers.parse_action_packet(act, messages)
    if parsed then
...
packethandlers.HandleIncomingPacket = function(e, messages)
    ...
    elseif (e.id == 0x28) then
        gPacketHandlers.HandleIncoming0x28(e, messages)
    end
