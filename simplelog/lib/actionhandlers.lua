...
actionhandlers.parse_action_packet = function(act)
    ...
    -- Restored from the supplied working SimpleLog reference.  This queue is
    -- intentionally separate from the chat formatter: it uses the original
    -- GDI warning objects and fades their text in the addon's render loop.
    if messages and gProfileSettings and gProfileSettings.mode
        and not gProfileSettings.mode.warnings
        and act.actor and act.actor.is_monster
        and (act.actor.owner == nil or act.actor.owner == 'other')
        and act.action and act.action.name and act.action.name ~= 'hit' then

        local action_name = act.action.name:gsub("[^%w%s%p]+", "")
        local warning_text = tostring(act.actor.name or 'Unknown') .. ' - ' .. action_name
        local is_priority = false

        for _, priority_action in ipairs(gPriority or {}) do
            if string.match(action_name, priority_action) then
                is_priority = true
                break
            end
        end
        ...
        for index = 1, 5 do
            if messages[index] and messages[index].expiry ~= nil
                and warning_text == messages[index].text then
                duplicate = true
                break
            end
        end

        local warning_settings = nil
        if SimpleLogWarningQueue then
            local _, settings = SimpleLogWarningQueue()
            warning_settings = settings
        end

        if warning_settings and not duplicate
            and (is_priority or not warning_settings.display_priority_only) then
            for index = 1, 5 do
                local entry = messages[index]
                if entry and entry.expiry == nil then
                    if is_priority then
                        local color = warning_settings.font_color_priority
                        if warning_settings.use_alt_priority_font_color then
                            color = warning_settings.font_color_priority_alt
                        end
                        entry.fontobj:set_font_color(color)
                    else
                        entry.fontobj:set_font_color(warning_settings.font_color_default)
                    end

                    entry.fontobj:set_opacity(1)
                    entry.fontobj:set_text(warning_text)
                    entry.text = warning_text
                    entry.expiry = os.clock() + warning_settings.fade_after
                    break
                end
            end
        end
...
actionhandlers.ActToString = function(original,act)
    ...
    react = assemble_bit_packed(react,act.target_count,72,78)
    react = assemble_bit_packed(react,act.result_count or 0,78,82)
...
actionhandlers.StringToAct = function(packet, packet_size)
    -- Ashita's current action parser reads a 6-bit target count at bit 72,
    -- followed by a separate 4-bit header value.  The prior SimpleLog fork
    -- consumed all 10 bits as the count, which produced invalid target loops.
    if not packet or string.byte(packet) ~= 0x28 then
        return nil
    end

    local bytes = packet:totable()
    local total_bits = (packet_size or #packet) * 8
    local function read(offset, width)
        if offset < 0 or width < 1 or (offset + width) > total_bits then
            return nil
        end
        return ashita.bits.unpack_be(bytes, offset, width)
    end

    -- A valid action header ends at bit 150.
    if total_bits < 150 then
        return nil
    end

    local act_table = {}
    act_table.size         = read(32, 8)
    act_table.actor_id     = read(40, 32)
    act_table.target_count = read(72, 6)
    act_table.result_count = read(78, 4)
    act_table.category     = read(82, 4)
    act_table.param        = read(86, 16)
    act_table.unknown      = read(102, 16)
    act_table.recast       = read(118, 32)
    act_table.msg          = total_bits >= 240 and read(230, 10) or 0
    act_table.targets      = {}

    if not act_table.size or not act_table.actor_id or not act_table.target_count
        or not act_table.category or not act_table.param or not act_table.recast then
        return nil
    end

    local offset = 150
    for _ = 1, act_table.target_count do
        -- Target server ID plus its 4-bit action count.
        if (offset + 36) > total_bits then
            return nil
        end

        local target = {
            offset_start = offset,
            server_id = read(offset, 32),
            action_count = read(offset + 32, 4),
            actions = {},
        }
        offset = offset + 36

        for _ = 1, target.action_count do
            -- Base result payload is 86 bits, followed by the spike-effect flag.
            if (offset + 87) > total_bits then
                return nil
            end

            local action = {
                offset_start = offset,
                reaction = read(offset, 5),
                animation = read(offset + 5, 12),
                effect = read(offset + 17, 4),
                stagger = read(offset + 21, 3),
                knockback = read(offset + 24, 3),
                param = read(offset + 27, 17),
                message = read(offset + 44, 10),
                unknown = read(offset + 54, 31),
            }

            action.has_add_effect = read(offset + 85, 1) == 1
            offset = offset + 86
            if action.has_add_effect then
                if (offset + 37) > total_bits then
                    return nil
                end
                action.add_effect_animation = read(offset, 6)
                action.add_effect_effect = read(offset + 6, 4)
                action.add_effect_param = read(offset + 10, 17)
                action.add_effect_message = read(offset + 27, 10)
                offset = offset + 37
            else
                action.add_effect_animation = 0
                action.add_effect_effect = 0
                action.add_effect_param = 0
                action.add_effect_message = 0
            end

            action.has_spike_effect = read(offset, 1) == 1
            offset = offset + 1
            if action.has_spike_effect then
                if (offset + 34) > total_bits then
                    return nil
                end
                action.spike_effect_animation = read(offset, 6)
                action.spike_effect_effect = read(offset + 6, 4)
                action.spike_effect_param = read(offset + 10, 14)
                action.spike_effect_message = read(offset + 24, 10)
                offset = offset + 34
            else
                action.spike_effect_animation = 0
                action.spike_effect_effect = 0
                action.spike_effect_param = 0
                action.spike_effect_message = 0
            end
...
actionhandlers.ActorParse = function (actor_id)
    ...
    if actor_table == nil then
        -- Entity memory can lag behind an action packet.  Keep a fully classified
        -- fallback so CheckFilter does not silently discard the cast/log message.
        return {
            name = ('{Unknown:%s}'):fmt(actor_id),
            id = actor_id,
            is_npc = true,
            type = 'mob',
            damage = 'mobdmg',
