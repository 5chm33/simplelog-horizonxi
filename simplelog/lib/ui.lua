...
ui.render_config = function(toggle)
    ...
    local draw_window = imgui.Begin(('SimpleLog - v%s'):fmt(addon.version), ui.state.open)
    ...
    if draw_window then
        imgui.BeginGroup()
            ...
            imgui.BeginChild('conf_box', { imgui.GetWindowWidth()-16, imgui.GetWindowHeight()-120 })
                imgui.PopStyleColor()
                if ui.state.tab == 0 then
                    local lang_choices = {'English', 'Japanese'}

                    -- Restored reference control.  The current binding takes a
                    -- fresh boolean reference table for every checkbox call.
                    local disable_warnings = imgui.Checkbox('##warnings', {gProfileSettings.mode.warnings == true})
                    if disable_warnings then
                        gProfileSettings.mode.warnings = not (gProfileSettings.mode.warnings == true)
                    end
                    imgui.SameLine()
                    imgui.TextColored(theme.header_text_col, 'Disable UI Warnings')
                    imgui.TextColored(theme.desc_text_col, 'Hide the centered monster spell and ability warnings.')
                    imgui.Separator()
